...
build_proot() {
    if [ ! -d "$PROOT_DIR/src" ]; then
        log_error "PRoot source missing at $PROOT_DIR. Initialize submodules with: git submodule update --init --recursive"
    fi
...
verify_artifacts() {
    ...
    # Dependency-closure check. Gradle will happily package a broken libproot.so
    # because it does not inspect DT_NEEDED. Every non-system dependency named
    # by libproot.so must be present beside it in jniLibs. This catches the
    # exact failure where libproot.so needs libandroid-shmem.so but the APK
    # contains only libtalloc.so.
    if command -v readelf >/dev/null 2>&1 && [ -f "$JNILIBS_BIN" ]; then
        local needed dep base
        while IFS= read -r dep; do
            case "$dep" in
                libc.so|liblog.so|libm.so|libdl.so|libc++_shared.so|libandroid.so|libz.so) continue ;;
            esac
            base="$dep"
            if [ ! -f "$JNILIBS_DIR/$base" ] && [ ! -f "$JNILIBS_DIR/${base%.so}.so" ]; then
    ...
    if [ "$failed" -ne 0 ]; then
        log_error "Sandbox native dependency verification failed. Refusing to produce an APK with an incomplete PRoot runtime."
    fi
