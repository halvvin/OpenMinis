...
object ShellExecutor {
    ...
    // currentProcess is a single shared slot. Serializing one-shot PRoot
    // executions prevents command B from overwriting the process handle that
    // command A would later destroy on timeout/cancellation.
    ...
    private suspend fun runOnce(
        ...
        if (PRootKernel.nativeLibDir.isNotEmpty()) {
            env["LD_LIBRARY_PATH"] = PRootKernel.runtimeLibraryPath(context)
        }
