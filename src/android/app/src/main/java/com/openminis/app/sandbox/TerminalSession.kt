...
class TerminalSession(private val context: Context) {
    ...
    companion object {
        ...
        /** Stop every live PTY before rootfs reset or process teardown. */
        fun stopAllLiveSessions() {
            val snapshot = liveSessions.toList()
            liveSessions.clear()
            snapshot.forEach { ref -> ref.get()?.stop() }
        }
        ...
        scope.launch {
            try {
                ...
                if (PRootKernel.nativeLibDir.isNotEmpty())
                    envMap["LD_LIBRARY_PATH"] = PRootKernel.runtimeLibraryPath(context)
